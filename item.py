...
class Item(Resource):
    ...
    parser.add_argument('store_id',
                     type = int,
                     required=True,
                     help = "Every items need store id"
                )
    @jwt_required()
    def get(self,name):
    ...
    def post(self,name): 
        if ItemModel.find_by_name(name):
            return {'message': "An item with name'{}' alerady exists".format(name)},  400
...
class Item_List(Resource):
    def get(self):
        return {'items': [x.json() for x in ItemModel.query.all()]}  ## or {'items}: list(map(lambda x:x.json(),ItemModel.query.all()))}
